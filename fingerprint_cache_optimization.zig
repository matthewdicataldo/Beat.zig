...
// Prediction Lookup Caching Optimization (Task: Optimization 1)
//
// This module implements high-performance caching for fingerprint registry lookups
// to address the 12% throughput overhead identified in COZ profiling results.
...
pub const CachedFingerprintRegistry = struct {
    ...
    hot_threshold: u32, // Minimum access count to be considered "hot"
    
    const Self = @This();
    
    pub const CacheConfig = struct {
        lru_cache_size: usize = 64,     // Recent lookups
        hot_cache_size: usize = 16,     // Frequently accessed fingerprints
        hot_threshold: u32 = 5,         // Accesses needed to be "hot"
    ...
    pub const InvalidationStrategy = enum {
        immediate,  // Invalidate on any update
        adaptive,   // Invalidate based on confidence change
        periodic,   // Invalidate periodically
    };
    
    pub const CachedPrediction = struct {
        predicted_cycles: f64,
        confidence: f32,        // Match PredictionResult type
        variance: f64,
        execution_count: u64,
        timestamp: u64,        // When this prediction was cached
        access_count: u32,     // How many times this has been accessed
        confidence_when_cached: f32, // To detect if confidence has changed significantly
    };
    ...
    pub fn getPredictionWithConfidence(self: *Self, fingerprint: beat.fingerprint.TaskFingerprint) beat.fingerprint.FingerprintRegistry.PredictionResult {
        ...
        if (self.hot_cache.get(fingerprint_hash)) |cached| {
            ...
            if (self.isCacheEntryValid(cached, current_time)) {
                ...
                self.hot_cache.recordAccess(fingerprint_hash); // Update frequency
        ...
        if (self.prediction_cache.get(fingerprint_hash)) |cached| {
            if (self.isCacheEntryValid(cached, current_time)) {
                ...
                if (cached.access_count >= self.hot_threshold) {
                    self.hot_cache.put(fingerprint_hash, cached) catch {}; // Best effort
        ...
        // Add to LRU cache
        self.prediction_cache.put(fingerprint_hash, cached) catch {}; // Best effort
    ...
    fn isCacheEntryValid(self: *Self, cached: CachedPrediction, current_time: u64) bool {
        ...
        if (age_ns > 5_000_000_000) return false;
        
        // For high-confidence predictions, allow longer cache time
        if (cached.confidence > 0.8) {
            if (age_ns > 10_000_000_000) return false; // 10 seconds for high confidence
        }
        
        // For very low confidence, expire quickly
        if (cached.confidence < 0.3) {
            if (age_ns > 1_000_000_000) return false; // 1 second for low confidence
        }
        
        // TODO: Could add confidence-change-based invalidation by checking current confidence
        _ = self;
        return true;
    }
    
    /// Invalidate cache entries for a specific fingerprint (called when profile is updated)
...
/// LRU Cache implementation optimized for fingerprint lookups
    ...
    return struct {
        ...
        pub fn remove(self: *Self, key: K) bool {
            if (self.map.get(key)) |node| {
                self.removeNode(@constCast(&node));
    ...
    return struct {
        ...
        fn evictLeastFrequent(self: *Self) void {
            ...
            while (iterator.next()) |entry| {
                // Prefer least frequent, then oldest
...
pub fn runCacheOptimizationTest(allocator: std.mem.Allocator) !void {
    std.debug.print("=== Prediction Lookup Cache Optimization Test ===\n", .{});
    ...
    if (pool.fingerprint_registry) |base_registry| {
        ...
        std.debug.print("Testing cache performance with {} lookup patterns...\n", .{1000});
        ...
        if (stats.getHitRate() > 0.7) {
            std.debug.print("✅ Cache optimization successful ({}% hit rate)\n", .{@as(u32, @intFromFloat(stats.getHitRate() * 100))});
        } else {
...
pub fn main() !void {
    ...
    try runCacheOptimizationTest(allocator);
}